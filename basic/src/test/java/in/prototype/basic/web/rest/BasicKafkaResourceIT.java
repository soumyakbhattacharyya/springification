...
import in.prototype.basic.config.EmbeddedKafka;
import in.prototype.basic.config.KafkaSseConsumer;
import in.prototype.basic.config.KafkaSseProducer;
...
class BasicKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/basic-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/basic-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/basic-kafka/unregister"));
                return;
            }
        }
